...
describe('Pair', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(
        () => new Pair(new TokenAmount(DAS, '100'), new TokenAmount(WAVAX[ChainId.AVALANCHE], '100'), ChainId.FUJI)
      ).toThrow('CHAIN_IDS')
    })
  })
  ...
  describe('#getAddress', () => {
    it('returns the correct address', () => {
      ...
      expect(Pair.getAddress(DAS, CON, ChainId.AVALANCHE)).toEqual('0x69DbDACFcab5b0fe783bCeD6fbaAdB636409fAC6')
    })
  })

  describe('#token0', () => {
    it('always is the token that sorts before', () => {
      expect(new Pair(new TokenAmount(DAS, '100'), new TokenAmount(CON, '100'), ChainId.FUJI).token0).toEqual(DAS)
      expect(new Pair(new TokenAmount(CON, '100'), new TokenAmount(DAS, '100'), ChainId.FUJI).token0).toEqual(DAS)
    })
  })
  describe('#token1', () => {
    it('always is the token that sorts after', () => {
      expect(new Pair(new TokenAmount(DAS, '100'), new TokenAmount(CON, '100'), ChainId.FUJI).token1).toEqual(CON)
      expect(new Pair(new TokenAmount(CON, '100'), new TokenAmount(DAS, '100'), ChainId.FUJI).token1).toEqual(CON)
    })
  })
  describe('#reserve0', () => {
  ...
  describe('#priceOf', () => {
    ...
    it('returns price of token in terms of other token', () => {
      expect(pair.priceOf(DAS)).toEqual(pair.token0Price)
      expect(pair.priceOf(CON)).toEqual(pair.token1Price)
    })
